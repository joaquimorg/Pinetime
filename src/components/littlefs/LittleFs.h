            ...
            public:
                class LittleFsFile {
                    public:
                        ...
                        int32_t read(void* buffer, size_t size) { return lfs_file_read(&mLfs, &mLfs_file, buffer, size); }
                        int32_t write(const void* buffer, size_t size) { return lfs_file_write(&mLfs, &mLfs_file, buffer, size); }
                        int32_t seek(size_t offset, size_t whence) { return 0u == lfs_file_seek(&mLfs, &mLfs_file, offset, whence); }
                        bool truncate(size_t sz) { return 0u == lfs_file_truncate(&mLfs, &mLfs_file, sz); }
                        /// return the current position within the file
                        int32_t tell() { return lfs_file_tell(&mLfs, &mLfs_file); }
                        int32_t size() { return lfs_file_size(&mLfs, &mLfs_file); }
                    private:
                        lfs_t& mLfs;
